...
@section('content')
    <div class="card">
        <div class="card-body">
            <p>{!! __('Warning, translations are not visible until they are exported back to the app/lang file, using <code>php artisan translation:export</code> command or publish button.') !!}</p>
            <div class="alert alert-success success-import" style="display:none;">
                <p>{!! __('Done importing, processed <strong class="counter">N</strong> items! Reload this page to refresh the groups!') !!}</p>
            </div>
            <div class="alert alert-success success-find" style="display:none;">
                <p>{!! __('Done searching for translations, found <strong class="counter">N</strong> items!') !!}</p>
            </div>
            <div class="alert alert-success success-publish" style="display:none;">
                <p>{!! __('Done publishing the translations for group \':group\'!', compact('group')) !!}</p>
            </div>
            ...
            @if(!isset($group))
                <form class="form-import" method="POST" action="{{ action('\Barryvdh\TranslationManager\Controller@postImport') }}" data-remote="true" role="form">
                    ...
                    <div class="form-group">
                        <div class="row">
                            <select name="replace" class="form-control form-select-sm">
                                <option value="0">{{ __('Append new translations') }}</option>
                                <option value="1">{{ __('Replace existing translations') }}</option>
                            </select>
                            <button type="submit" class="btn btn-success btn-block mt-2"  data-disable-with="Loading..">{{ __('Import groups') }}</button>
                        </div>
                    </div>
                </form>
            @endif
            @if(isset($group))
                <form class="form-inline form-publish" method="POST" action="{{ action('\Barryvdh\TranslationManager\Controller@postPublish', $group) }}" data-remote="true" role="form" data-confirm="{{ __('Are you sure you want to publish the translations group \':group\'? This will overwrite existing language files.', compact('group')) }}">
                    @csrf
        ...
        <div class="card-footer">
            <form class="form-find" method="POST" action="{{ action('\Barryvdh\TranslationManager\Controller@postFind') }}" data-remote="true" role="form" data-confirm="{{ __('Are you sure you want to scan you app folder? All found translation keys will be added to the database.') }}">
                @csrf
    ...
    <div class="card">
        <form role="form" method="POST" action="{{ action('\Barryvdh\TranslationManager\Controller@postAddGroup') }}">
            ...
            <div class="card-footer">
                <input type="submit" class="btn btn-primary" name="add-group" value="{{ __('Add and edit keys') }}" />
            </div>
        </form>
    </div>
    @if($group)
        ...
        <div class="row">
            <div class="col-sm-2">
                <span class="btn btn-default enable-auto-translate-group">{{ __('Use Auto Translate') }}</span>
            </div>
        </div>
        <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form" action="{{ action('\Barryvdh\TranslationManager\Controller@postTranslateMissing') }}">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        <label for="base-locale">{{ __('Base Locale for Auto Translations') }}</label>
                    ...
                    <div class="form-group">
                        <label for="new-locale">{{ __('Enter target locale key') }}</label>
                    ...
                    @if(!config('laravel_google_translate.google_translate_api_key'))
                        <p>
                            {!! __('<code>If you would like to use Google Translate API, install tanmuhittin/laravel-google-translate and enter your Google Translate API key to config file laravel_google_translate</code>') !!}
                        </p>
                    @endif
                    <div class="form-group">
                        <input type="hidden" name="with-translations" value="1">
                        <input type="hidden" name="file" value="{{ $group }}">
                        <button type="submit" class="btn btn-default btn-block"  data-disable-with="Adding..">{{ __('Auto translate missing translations') }}</button>
                    </div>
                </div>
            </div>
        </form>
        <div class="card">
            <div class="card-header">
                <h4>{{ __('Total: :total, changed: :changed', ['total' => $numTranslations, 'changed' => $numChanged]) }}</h4>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-12">
                                        ...
                                        @if ($deleteEnabled)
                                            <td>
                                                <a href="{{ action('\Barryvdh\TranslationManager\Controller@postDelete', [$group, $key]) }}"
                                                   class="delete-key"
                                                   data-confirm="{{ __('Are you sure you want to delete the translations for \':key\'?', compact('key')) }}"
                                                >
    ...
    @else
        ...
        <fieldset>
            ...
            <form class="form-inline form-publish-all" method="POST" action="{{ action('\Barryvdh\TranslationManager\Controller@postPublish', '*') }}" data-remote="true" role="form" data-confirm="{{ __('Are you sure you want to publish all translations group? This will overwrite existing language files.') }}">
                @csrf
